...
./run.sh xa
    ...
    \subsection{编译脚本 (run.bat 和 run.sh)}\label{sub:scripts}

    为了简化 LaTeX 文档的编译过程，模板提供了两个编译脚本，分别用于 Windows 和 Linux/macOS 系统：

    \begin{itemize}
        \item \textbf{Windows (run.bat)}：双击此批处理文件可以一键编译生成 PDF 文档。此脚本的主要目的是帮助不熟悉 LaTeX 编译过程的初学者快速上手。\textbf{注意：请勿通过邮件传播和接收此脚本，以防范 DOS 脚本的潜在安全风险。}

        \item \textbf{Linux/macOS (run.sh)}：在终端中运行以下命令：
              \begin{itemize}
                  \item \verb|./run.sh xa <filename>|：执行完整编译流程，生成最终 PDF 文档
                  \item \verb|./run.sh x <filename>|：执行快速编译模式，适用于内容修改但无新引用的情况
              \end{itemize}
    \end{itemize}
